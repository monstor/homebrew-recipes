# Orange Glow · DDH NEIPA

> 高強度雙乾投 NEIPA · 套用妃子起笑 hop creep 失控教訓

## 📊 當前狀態

| 項目 | 規格 |
|---|---|
| **目前最新版本** | [v2.5](./v2.5-clean-neipa.html)(清爽版) |
| 風格 | Clean NEIPA(清爽版;v2.4 為厚實 DDH) |
| 容量 | 10 L |
| OG | 1.062-1.066 |
| FG | 1.010-1.012 |
| ABV | ~7.0% |
| IBU | ~20 |
| EBC | 10-12 |
| 基麥 | Golden Promise 3.0kg(+ Munich 0.5 + 小麥 0.9,無燕麥) |
| 酵母 | LalBrew Pomona 1 包 → 高壓純氧直投(免 starter;保險可 2 包 / 養 2L starter) |
| 可發酵糖 | 無(v2.5 不加 DME/Maltodextrin,走清爽) |
| 添加物 | Fermaid-O 5g（投酵母 2.5g + 投後 24h 2.5g;備選 Fermaid-K 3g 煮沸末）+ Maturex L (ALDC) 0.2-0.3ml |
| 抗氧化劑 | **轉桶時進 keg:L-抗壞血酸 0.3g + 焦亞硫酸鉀 KMS 0.25g（必須成對，VC 不可單用）** |

## 🍻 版本歷史

### v1 - Original([檔案](./v1-original.html))
原始配方。OG 1.078 / ABV 7.8-8.2% 的高強度 DDH NEIPA,使用 Extra Pale Ale Malt 取代 Maris Otter,兩段 Dry Hop(Day 4-5 + Day 8-9 冷崩前)。

**潛在問題(基於妃子起笑 v1 的教訓推論):**
- 第二段 Dry Hop 在 FG 已到位才投,跟妃子起笑 v1 同樣失控路徑
- OG 1.078 糊精多,hop creep 啃糊精會比妃子起笑(FG 1.014→1.008)更兇
- 8% ABV 酵母疲勞,雙乙醯清除能力差
- 雙包 Verdant 成本高

**未實際釀造,直接升級到 v2。**

---

### v2 - Lessons Learned Integration([檔案](./v2-lessons-learned.html))
**核心:全面套用妃子起笑實戰教訓**

13 項更新:

1. 單段 Dry Hop 卡進主發酵期(Day 3-4,比重 1.045-1.035 時投全部 112g)
2. 綠燈確認 SOP(連兩次比重不動 + 強迫雙乙醯測試)
3. 強迫雙乙醯測試方法
4. 一包 Verdant + 2L Starter(取代 2 包,省成本)
5. Fermaid-K → Fermaid-O(不抑制酯類,保留 NEIPA 果香)
6. 省略 Cold Crash
7. 虹吸轉桶完整 SOP
8. 低壓慢飽和強制碳酸(禁止 30 PSI 猛搖)
9. 取消瓶內二發選項(8% ABV 酵母太疲勞)
10. 時程延長 14 天 → 18-20 天
11. 最佳賞味期 2-3 週警告
12. 糖化降到 65°C
13. Day 3 比重檢查 fallback 邏輯

---

### v2.1 - Starter Fix + ALDC Integration([檔案](./v2.1-starter-fix-aldc.html))
**核心:v2 的兩個小修正**

1. **修正 Starter 容器尺寸**:v2 原本錯誤寫「2L 玻璃瓶」,正確應該是「4-5L 容器裝 2L 麥汁」(泡沫會佔麥汁高度 30-50%)。
   - 容器選項:4L 廣口玻璃罐(200-400 元)/ 5L 硼矽酸鹽錐形瓶(800-1500 元)/ 4L 大泡菜罐(200 元)

2. **整合 ALDC(Maturex L)SOP**:投酵母當下加 0.2-0.3ml,從化學源頭阻斷雙乙醯生成。
   - 取量方法:胰島素注射器(藥局幾十塊,1ml 細分 100 格)或稀釋法(10% 濃度)
   - 對 DDH/高 ABV 特別有價值
   - Fermaid-O 在 v2.1 改回 Fermaid-K(因為買不到 Fermaid-O)

**v2 的其他核心改造(單段 Dry Hop、綠燈確認、虹吸轉桶、低壓慢飽和、省略 Cold Crash 等)全部保留。**

---

### v2.2 - Whirlpool + Fermaid Tuning([檔案](./v2.2-whirlpool-fermaid.html))
**核心:兩個往「更果汁、更柔和」方向的風味微調 + 一項數字修正**

不動發酵流程,只調兩個直接影響 NEIPA 果香的細節:

1. **旋渦溫度 80°C → 75°C**:高溫旋渦會稍微烤掉嬌嫩的酒花精油、也多帶苦。降到 75°C 香氣保留更好、苦感更柔,IBU 更穩定落在 25-30 低區(75°C 仍在巴氏滅菌區間,食安沒問題)
2. **營養劑改 Fermaid-O 5g 為主**:Fermaid-O 是純有機氮源、不含 DAP,不會抑制果香酯類(商業 NEIPA 廠首選)。v2.1 因買不到才退用 Fermaid-K,v2.2 改回 Fermaid-O 為主、**Fermaid-K 降為備選**(買不到 O 時用 3g)
3. **修正酒花總量**:從 v1 起表頭一直誤寫「168g」,實際是 Magnum 6 + 旋渦 56 + Dry Hop 112 = **174g**(漏算了苦花 Magnum 6g)

**v2.1 的其他設計(Starter 4-5L 修正、ALDC SOP、單段 Dry Hop、綠燈確認、虹吸轉桶、低壓慢飽和)全部保留。**

---

### v2.3 - DME Base + Less Oats([檔案](./v2.3-dme.html))
**核心:換厚度來源 —— DME 取代葡萄糖 + 燕麥減量**

配方/流程變更,其餘完全沿用 v2.2:

- **葡萄糖 200g → 大麥 DME 220g**:葡萄糖 100% 可發酵、把酒拉乾;DME 約 75-80% 可發酵,留糊精與麥芽風味 → 麥香骨架。starter 本來就用 DME,**一物兩用、免另買葡萄糖**
- **燕麥 1000g → 800g(Extra Pale 維持 2000g)**:減 β-聚葡萄糖 → 口感不要太油滑、糖化洗槽更順、降卡槽風險。穀物降到 4.8kg,OG 可能略低 1-2 點(可接受,或靠 DME/補基礎麥芽拉回)
- **Fermaid-O 改投酵母時加(不煮)、分兩次**:有機氮要靠酵母在發酵期分段吸收,煮沸用不到那優勢。投酵母當下 2.5g 跟 ALDC 一起撒入、投後約 24h 再補 2.5g(更貼酵母需氮曲線,高 OG + 單包時發酵更穩、果香更飽)。(備選 Fermaid-K 才維持煮沸末加,因 DAP 即時可用)
- **整體走向**:從「滑而厚」變「乾淨有支撐」——DME 的麥香骨架取代葡萄糖的乾瘦,燕麥減量收掉油滑,兩者方向一致
- **數據**:FG 1.015-1.017、ABV 7.7-8.1%、EBC 13-15(略深);OG 維持 1.075-1.078;haze 有小麥+酒花撐著幾乎不變
- 旋渦 75°C、單段 Dry Hop、Starter、ALDC、綠燈、虹吸轉桶、低壓慢飽和全部不變
- 微調方向:偏厚/偏甜 → DME 降回 ~180g;太瘦/haze 不夠 → 燕麥加回 900-1000g

---

### v2.4 - Golden Promise Base + LalBrew Pomona([檔案](./v2.4-golden-promise.html))
**核心:基麥換 Golden Promise + 酵母換 LalBrew Pomona,整體更英系 juicy、果香更外放;主穀整理成 1kg 整數單位、燕麥 0.8kg 控酒體**

動基麥與酵母,其餘(糖化 65°C、旋渦 75°C、Fermaid-O、單段 Dry Hop、ALDC、綠燈、虹吸轉桶、低壓慢飽和、**酒花配方 Citra/Amarillo/Galaxy**)完全沿用 v2.3:

- **Extra Pale 2kg → Golden Promise 2kg(1:1 換基麥)**:GP 是蘇格蘭傳統 2-row,蛋白質略高、帶餅乾/麵包柔甜蜜香 → 酒體更柔順綿密、泡沫更飽滿。英系 NEIPA 廠(Verdant、Cloudwater)的愛用基底
- **酵母 Verdant IPA → LalBrew Pomona(乾酵母)**:Lallemand × Escarpment 的現代 hybrid IPA 酵母,專為高 ABV、重酒花 IPA 設計。**高生物轉化**——把乾投酒花的硫醇前驅物轉成桃/柑橘/熱帶香、高發酵度但不啃糊精、穩定 haze、耐操。跟本配方「發酵期單段乾投」是絕配:酵母一邊發酵一邊做生物轉化,把 Citra/Galaxy 的橘香與熱帶感放得比 Verdant 更外放。**⚠️ 但 Pomona 發酵極快(首批 #0008 第 3 天就 1.016),1.045-1.035 的乾投窗口約落在 Day 1-2,要比 Verdant(Day 3-4)更早乾投——從 Day 1 起就量比重、進區間立刻投,別等 Day 3。****標準走 1 包 + 投酵母前高壓純氧 60-90s 直投、免 starter**;想保險再上 2 包或養 2L starter
- **主穀 1kg 整數 + 燕麥 0.8kg:GP 2 / Munich 1 / Wheat 1 / 燕麥 0.8 = 4.8kg**:GP/Munich/Wheat 取整好採購,燕麥維持 0.8kg 收掉油滑、避免太厚太甜(Munich 撐橘金酒色與麥香層次)
- **大麥 DME 維持 180g(煮沸末,以 g 計、非麥芽)**:跟 starter 共用、補一點 OG。穀物 4.8kg + DME,OG 維持 v2.3 水準
- **整體走向**:GP 給「柔順綿密、泡沫飽滿」的英系酒體,Pomona 把熱帶柑橘果香放大、haze 更穩定。橘香主軸(Citra/Amarillo/Galaxy)不變、反而更外放
- **數據**:OG 1.075-1.078、FG 1.015-1.017、ABV 7.7-8.1%、EBC 14-16(GP 略深)
- 微調方向:想更厚更綿密/泡沫更持久 → 燕麥加回 1kg;想更乾淨的酒花清晰度 → GP 與 Extra Pale 各半

### v2.5 - Clean NEIPA([檔案](./v2.5-clean-neipa.html)) ★ 當前版本
**核心:清爽版 NEIPA + 全面修正 #0007 過苦**

從 v2.4「高 OG 厚實 DDH」轉向乾淨清爽、低苦(~20 IBU),香氣靠 Whirlpool + Dry hop 疊:

- **酒花處理(最大改變,修 #0007 太苦)**:苦花 Magnum 6g 準時 60min 加、裝濾袋、**煮完撈出**(#0007 忘加→補煮一小時 + 沒撈是過苦主因);**Whirlpool 列為必做** 71-82°C(目標 78°C)大量香花 84g(濾袋)靜置 20-30min 後撈出(#0007 疑似漏做→香氣不足);**Dry hop 84g 限時 3-5 天**就撈出/轉桶(#0007 泡到發酵完→冷側粗糙苦 humulinone/多酚)
- **Dry hop 時機看比重**:OG 降一半(~1.037)時投,不看固定天數——避開 #0008 太晚投、biotransformation 打折
- **穀物走清爽**:去燕麥(太絲滑)、只留小麥 ~20%(Château Wheat Blanc);不加 DME/Maltodextrin;糖化 67→65°C(發酵度高、酒體清爽)。GP 3.0 / Munich 0.5 / Wheat 0.9 = 4.4kg
- **水質**:氯化物主導 CaCl₂:石膏 ≥ 2:1(圓潤、平衡苦);**pH 計換 SR44 電池**、校正、確實調糖化 pH 5.2-5.4
- **抗氧化劑(封裝端新增)**:轉桶時在接收 keg 底部先放 **L-抗壞血酸 0.3g(30 mg/L)+ 焦亞硫酸鉀 KMS 0.25g(25 mg/L)**,溶進 20-30ml 冷開水 → purge ×3 → 底部入酒。**兩者必須成對**:抗壞血酸抓氧時產生 H₂O₂,遇酒中微量鐵/銅會走 **Fenton 反應**生成羥自由基 → **氧化得比不加還兇**;KMS 的工作就是收掉 H₂O₂。**只有 VC 沒有 KMS → 寧可兩個都不加。**
  - 不會有硫味:SO₂ 進酒大半立刻結合成無味的結合態,14 ppm 總 SO₂ 遠低於感知門檻(~20-25 ppm 游離),且 Ale 酵母本來就自產 5-20 ppm。**別超過 KMS 0.5g/10L**
  - **藥劑是保險(20%),流程才是主力(80%)**:keg 三次 CO₂ purge、底部入酒不濺、頂空最小、趁鮮喝。流程沒做好,加再多藥也救不回
  - 走**強制碳酸無影響**;若改瓶內二發要小心亞硫酸會壓制疲勞酵母
- **沿用不變**:Pomona 1 包 + 純氧直投、ALDC、低氧虹吸轉桶、控溫 18-20°C、綠燈確認、趁鮮喝(碳酸完成後 1-2 週內)
- **數據**:OG 1.062-1.066、FG 1.010-1.012、ABV ~7.0%、IBU ~20、EBC 10-12
- 微調:想更厚 → 燕麥加回 0.5-0.8kg;想更烈 → 補 DME 拉 OG;想更苦 → whirlpool 往 82°C 或苦花加到 8-10g

## 📝 實戰追蹤(尚未釀造)

| 批次 | Starter 狀態 | OG | Day 3 比重 | Dry Hop 投放日 | 最終 FG | ALDC 用量 | 雙乙醯測試 | 賞味期實測 |
|---|---|---|---|---|---|---|---|---|
| | | | | | | | | |

## 🛠️ 釀造前準備清單

- [ ] 4-5L Starter 容器(廣口玻璃罐或錐形瓶)
- [ ] DME 乾麥芽萃取物 ~380g(starter 200g + 煮沸末 180g,一物兩用)
- [ ] 鋁箔紙(廚房用即可)
- [ ] Maturex L (ALDC) 100ml + 胰島素注射器
- [ ] Fermaid-O 5g(買不到改 Fermaid-K 3g)
- [ ] **抗氧化劑(封裝端):L-抗壞血酸 0.3g + 焦亞硫酸鉀 KMS 0.25g**
  - KMS 也可用 **Campden 錠(10L 半錠、壓碎溶開)**；**各廠劑量不同，依包裝標示重算**
  - ⚠️ **VC 必須配 KMS，不可單用**——抗壞血酸抓氧會產生 H₂O₂，遇酒中微量鐵/銅走 Fenton 反應生成羥自由基，**比不加更糟**。KMS 負責收掉 H₂O₂
  - 需 **0.01g 精度電子秤**；沒有就用 Campden 錠
- [ ] LalBrew Pomona 1 包(乾酵母;省 starter 則備 2 包)
- [ ] 麥芽:Golden Promise 2kg + Munich Light 1kg + Wheat 1kg + 燕麥片 0.8kg(主穀 1kg 整數)+ 大麥 DME 180g(煮沸末)
- [ ] 酒花:Magnum 6g + Citra 56g + Amarillo 56g + Galaxy 56g = 174g 總計
- [ ] 礦物鹽:氯化鈣 1.5g + 石膏 0.5g（偏氯化物 juicy）+ 磷酸（少量備用下調 mash pH）
- [ ] 水質假設：新北市淡水區自來水 + 櫻花 SAKURA 淨水（F0271 礦纖除菌 + F0231 活性碳除氯，**不軟化水**）→ pH 6.35 / KH 40 / GH 25 / 無餘氯（軟水低鹼微酸，為淡水區原水）。鈣偏低故 CaCl₂ 上修到 1.5g；低鹼故 mash pH 多半自落 5.3-5.5、輕調或免調。**換水源要重量 KH/GH 再算**

## 🔮 待決事項(等首批釀完評估)

- [ ] 7-8% ABV + DDH 強度是否如預期?
- [ ] ALDC 是否成功阻斷雙乙醯?
- [ ] Starter 操作流暢度
- [ ] 賞味期實測是否真的 2-3 週?
